...
class PaperCanvas extends Component {
  constructor(props) {
    ...
    this.drawHomeData = (origin) => {
      ...
      const measure = this.originMax - 7 + this.props.questionvalues[4] * 2;
      const config = {
        radius: this.radius,
        steps: measure,
      ...
      stepDiamond.draw();
        ...
        for (let i = 0; i < 1; i++) {
          ...
          if (this,props.questionvalues[2] !== 0) {
            const path = new StepPathTwo(
          ...
          if (this.props.questionvalues[3] !== 0) {

            let cols;
            if (this,props.questionvalues[2] > 0) {
              cols = [colors[this.props.questionvalues[2]]];
            } else {
              cols = colors;
            }
          ...
          if (this.props.questionvalues[5] !== 0) {

            const vConf = {
              radius: this.radius,
              steps: this.props.questionvalues[5] ,
              colors,
              offset: {
          ...
          if (this.props.questionvalues[6] !== 0) {

            const vConf = {
              radius: this.radius,
              steps: this.props.questionvalues[6] - 2,
              colors:[colors[3], colors[2]],
              offset: {
                x: - this.props.questionvalues[6] * this.radius + 2 * this.radius,
                y: 0
              }
            };
            const end = path.path.getPointAt(path.path.length);
          ...
          if (this.props.questionvalues[7] !== 0) {

            const vConf = {
              radius: this.radius,
              steps: this.props.questionvalues[6] - 1,
              colors:[colors[3], colors[2]],
              offset: {
                x: - this.props.questionvalues[6] * this.radius / 2,
                y: 0
              }
            };
            const end = path.path.getPointAt(path.path.length);
          ...
          if (this.props.questionvalues[8] !== 0) {

            const end = path.path.getPointAt(path.path.length);

            const vConf = {
              radius: this.radius,
              steps:this.props.questionvalues[8] - 3,
              colors: [colors[3]],
              offset: {
                x: this.originMax * this.radius + this.props.questionvalues[7]/2 * this.radius - 2 * this.radius,
                y: 0
              }
            };
  ...
  render() {
    ...
    return (
        <canvas className='' ref={this.canvasRef} {...this.props} id="canvas" resize="true" />
    )
  }
}
